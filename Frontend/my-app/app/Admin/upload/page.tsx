...
interface UploadResult { success: boolean; message: string; data: unknown[]; }

const SUBJECT_ICONS: Record<string, string> = {
  science:"⚗️",mathematics:"📐",history:"🏛️",geography:"🌍",technology:"💻",
  sports:"⚽",entertainment:"🎬",literature:"📚",maths:"📐",ai:"🤖",default:"🧠",
};
...
interface CustomDropdownProps {
  options: DropdownOption[]; value: string; onChange: (id: string) => void;
  placeholder: string; disabled?: boolean; showIcons?: boolean;
}
...
function CustomDropdown({ options, value, onChange, placeholder, disabled, showIcons }: CustomDropdownProps) {
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      <div className={`bu-select-btn${open?" open":""}${disabled?" disabled":""}`}
        onClick={() => !disabled && setOpen((o) => !o)} tabIndex={disabled?-1:0}
        ...
        {showIcons && <span className="bu-select-icon">{selected?getIcon(selected.name):"📁"}</span>}
        <span className="bu-select-text">{selected?selected.name:placeholder}</span>
        <svg className="bu-chevron-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 12 15 18 9"/></svg>
      </div>
      {open && (
        <div className="bu-dropdown-menu" role="listbox">
          <div className={`bu-option${!value?" selected":""}`} role="option" onClick={() => { onChange(""); setOpen(false); }}>
          ...
          {options.map((opt) => (
            <div key={opt.id} className={`bu-option${value===opt.id?" selected":""}`} role="option" aria-selected={value===opt.id}
...
export default function BulkUpload() {
  ...
  const getToken = () => document.cookie.split("; ").find((r)=>r.startsWith("session_token="))?.split("=")[1];

  const fetchSubjects = async () => {
    const res = await fetch(`${API}/api/getSubjects`,{headers:{Authorization:`Bearer ${getToken()}`}});
    const data = await res.json();
    setSubjects(data.data||[]);
  };

  const fetchTopics = async (subject_id: string) => {
    const res = await fetch(`${API}/api/topicBySUb/${subject_id}`,{headers:{Authorization:`Bearer ${getToken()}`}});
    const data = await res.json();
    const arr = data.data ? (Array.isArray(data.data)?data.data:[data.data]) : [];
    setTopics(arr); setSelectedTopic("");
  };
  ...
  const handleAddSubject = async () => {
    ...
    const res = await fetch(`${API}/api/insertSubject`,{method:"POST",headers:{Authorization:`Bearer ${getToken()}`,"Content-Type":"application/json"},body:JSON.stringify({name:newSubject})});
    const data = await res.json();
    if(data.data){setNewSubject("");await fetchSubjects();setSelectedSubject(data.data.subject_id);}
  };

  const handleAddTopic = async () => {
    if(!newTopic.trim()||!selectedSubject)return;
    const res = await fetch(`${API}/api/insertTopic`,{method:"POST",headers:{Authorization:`Bearer ${getToken()}`,"Content-Type":"application/json"},body:JSON.stringify({name:newTopic,subject_id:selectedSubject})});
    const data = await res.json();
    if(data.data){setNewTopic("");setSelectedTopic(data.data.topic_id);await fetchTopics(selectedSubject);}
  };

  const uploadQuestions = async (questions: Record<string, string>[]) => {
  const withTopic = questions.map((q)=>({...q,topic_id:selectedTopic}));
  ...
  const handleJSONUpload = async () => {
    ...
    try {
      ...
      if(!questions||!Array.isArray(questions)){setError("JSON must be an array of questions");setLoading(false);return;}
      const data = await uploadQuestions(questions);
      setResult(data);
    } catch(err) {
      setError(err instanceof SyntaxError ? "Invalid JSON — check your formatting." : err instanceof Error ? err.message : "Upload failed. Check your connection.");
  ...
  return (
    <>
      <style>{`
        ...
        .bu-select-btn{display:flex;align-items:center;gap:8px;font-family:'Outfit',sans-serif;font-size:14px;font-weight:600;color:#1e1b4b;background:#fff;border:1.5px solid #e0e7ff;border-radius:12px;padding:10px 14px;cursor:pointer;transition:border-color .18s,box-shadow .18s;width:100%;user-select:none;outline:none;}
        ...
        .bu-dropdown-menu{position:absolute;top:calc(100% + 6px);left:0;right:0;background:#fff;border:1.5px solid #e0e7ff;border-radius:14px;box-shadow:0 8px 28px rgba(99,102,241,.14);z-index:200;max-height:220px;overflow-y:auto;overflow-x:hidden;animation:budd .15s ease;}
        ...
        .bu-input{flex:1;font-family:'Outfit',sans-serif;font-size:14px;font-weight:500;color:#1e1b4b;background:#fff;border:1.5px solid #e0e7ff;border-radius:12px;padding:10px 14px;outline:none;transition:border-color .18s,box-shadow .18s;}
        ...
        .bu-btn-add{font-family:'Outfit',sans-serif;font-size:13px;font-weight:700;color:#fff;background:linear-gradient(135deg,#6366f1,#8b5cf6);border:none;border-radius:12px;padding:10px 18px;cursor:pointer;transition:opacity .18s,transform .15s;white-space:nowrap;}
        ...
        .bu-textarea{width:100%;font-family:'Outfit',monospace;font-size:13px;color:#1e1b4b;background:#fafbff;border:1.5px solid #e0e7ff;border-radius:14px;padding:16px;height:180px;resize:none;outline:none;transition:border-color .18s,box-shadow .18s;}
        .bu-textarea:focus{border-color:#6366f1;box-shadow:0 0 0 3px rgba(99,102,241,.1);}
        .bu-textarea::placeholder{color:#c4b5fd;}
        .bu-btn-upload{margin-top:14px;font-family:'Outfit',sans-serif;font-size:14px;font-weight:700;color:#fff;background:linear-gradient(135deg,#6366f1,#8b5cf6);border:none;border-radius:12px;padding:11px 28px;cursor:pointer;transition:opacity .18s,transform .15s;box-shadow:0 4px 14px rgba(99,102,241,.25);}
        ...
        .bu-file-label{display:inline-flex;align-items:center;gap:8px;cursor:pointer;font-family:'Outfit',sans-serif;font-size:13px;font-weight:700;color:#6366f1;background:#eef2ff;border:1.5px solid #c7d2fe;border-radius:10px;padding:8px 16px;transition:background .15s;}
      ...
      <div className="bu-page">
        <div className="bu-header">
          <div className="bu-header-inner">
            <div className="bu-tag"><span className="bu-tag-dot"/>Admin Panel</div>
            <h1 className="bu-title">Bulk <em>Upload</em> Questions</h1>
        ...
        <div className="bu-body">
          ...
          <div className="bu-card">
            ...
            <div className="bu-grid">
              <div>
                ...
                <CustomDropdown options={subjects.map(s=>({id:s.subject_id,name:s.name}))} value={selectedSubject} onChange={setSelectedSubject} placeholder="Pick a subject…" showIcons/>
              </div>
              <div>
                <span className="bu-field-label">Add New Subject</span>
                <div className="bu-add-row">
                  <input className="bu-input" type="text" value={newSubject} onChange={e=>setNewSubject(e.target.value)} placeholder="Subject name" onKeyDown={e=>e.key==="Enter"&&handleAddSubject()}/>
                  <button className="bu-btn-add" onClick={handleAddSubject} disabled={!newSubject.trim()}>Add</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bu-card">
            ...
            <div className="bu-grid">
              <div>
                ...
                <CustomDropdown options={topics.map(t=>({id:t.topic_id,name:t.name}))} value={selectedTopic} onChange={setSelectedTopic} placeholder={selectedSubject?"Pick a topic…":"Select subject first"} disabled={!selectedSubject}/>
              </div>
              <div>
                <span className="bu-field-label">Add New Topic</span>
                <div className="bu-add-row">
                  <input className="bu-input" type="text" value={newTopic} onChange={e=>setNewTopic(e.target.value)} placeholder="Topic name" disabled={!selectedSubject} onKeyDown={e=>e.key==="Enter"&&handleAddTopic()}/>
                  <button className="bu-btn-add" onClick={handleAddTopic} disabled={!newTopic.trim()||!selectedSubject}>Add</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bu-card">
            ...
            <textarea className="bu-textarea"
              placeholder='[{"question_text":"...","option_a":"...","option_b":"...","option_c":"...","option_d":"...","correct_answer":"full option text","difficulty":"easy"}]'
              value={jsonText} onChange={e=>setJsonText(e.target.value)}/>
            <button className="bu-btn-upload" onClick={handleJSONUpload} disabled={loading||!jsonText||!selectedTopic}>
          ...
          <div className="bu-card">
            ...
            <p className="bu-file-hint">Headers: question_text, option_a, option_b, option_c, option_d, correct_answer, difficulty<br/>correct_answer should be full option text, not "option a"</p>
            ...
            <br/>
            <button className="bu-btn-upload green" onClick={handleCSVUpload} disabled={loading||!file||!selectedTopic}>
          ...
          {result && (
            <div className="bu-alert-ok">
              ...
              <p className="bu-alert-ok-sub">Inserted: {result.data?.length??0} question(s)</p>
            </div>
          )}
        </div>
      </div>
