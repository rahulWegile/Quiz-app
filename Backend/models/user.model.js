...
//update user 
export const updateUser = async (id, name, email, password, gender, profile_url) => {
  try {
    const Email = email?.trim().toLowerCase();
    ...
    const { rows } = await query(sql, [name, Email, hashPass, gender, profile_url, id]);
    return rows[0];
...
export const insertOtp = async (email, code) => {
  try {
    const sql = `
      UPDATE users
