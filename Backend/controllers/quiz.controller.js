...
export const startQuiz = async (req, res) => {
  ...
  try {
    ...
    if (questions.length < 10) {
      return res
        .status(400)
        .json({ success: false, message: "Not enough questions available" });
    }
...
export const getSession = async (req, res) => {
  ...
  try {
    ...
    if (session.status === "completed") {
      return res
        .status(400)
        .json({ success: false, message: "Session has ended" });
    }
...
export const createScheduleQuiz = async (req, res) => {
  ...
  if (!topic_id || !scheduled_at) {
    return res
      .status(400)
      .json({ success: false, message: "topic_id and scheduled_at required" });
  }
  try {
    const session = await quiz.createScheduledSession(
      topic_id,
      scheduled_at,
      created_by,
    );
    ...
    if (delay > 0) {
      ...
      console.log(
        `⏳ Session ${session.id} scheduled to open in ${Math.round(delay / 1000)}s`,
      );
    }
...
export const registerQuiz = async (req, res) => {
  ...
  try {
   ...
   const REGISTRATION_CLOSE_MS = 2 * 60 * 1000;
const REGISTRATION_CLOSE_MIN = REGISTRATION_CLOSE_MS / 60_000;  // derives from source of truth

const twoMinBefore = new Date(scheduledAt.getTime() - REGISTRATION_CLOSE_MS);

if (now >= twoMinBefore) {
  return res.status(400).json({
    success: false,
    message: `Registration closed — starts in less than ${REGISTRATION_CLOSE_MIN} minutes`,
  });
}
